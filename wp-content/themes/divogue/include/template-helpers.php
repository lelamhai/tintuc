...
function hoot_header_branding() {
	...
	<div <?php hybridextend_attr( 'branding' ); ?>>
		<div id="site-logo" class="<?php
			...
			elseif ( hoot_get_mod('logo_background_type') == 'contrast' )
				echo ' contrast-typo with-background';
...
function hoot_header_aside() {
	?><div <?php hybridextend_attr( 'header-aside', '', 'header-aside-custom' ); ?>><?php
...
function hoot_header_menu() {
	...
	$class = ( $site_layout == 'boxed' ) ? 'grid' : 'grid-stretch';
	?>
...
function hoot_update_browser() {
	$notice = '<!--[if lte IE 9]><p class="chromeframe">' .
			  sprintf( __( 'You are using an outdated browser (IE 8 or before). For a better user experience, we recommend %1supgrading your browser today%2s or %3sinstalling Google Chrome Frame%4s', 'divogue' ), '<a href="http://browsehappy.com/">', '</a>', '<a href="http://www.google.com/chromeframe/?redirect=true">', '</a>' ) .
			  '</p><![endif]-->';
...
function hoot_meta_info_blocks( $args = '', $context = '' ) {
	...
	<div class="entry-byline">
		...
		if ( !empty( $display['author'] ) ) :
			$blocks['author']['label'] = __( 'By:', 'divogue' );
			ob_start();
		...
		if ( !empty( $display['date'] ) ) :
			$blocks['date']['label'] = __( 'On:', 'divogue' );
		...
		if ( !empty( $display['cats'] ) ) :
			...
			if ( !empty( $category_list ) ) :
				$blocks['cats']['label'] = __( 'In:', 'divogue' );
				$blocks['cats']['content'] = $category_list;
			endif;
		endif;

		if ( !empty( $display['tags'] ) && get_the_tags() ) :
			$blocks['tags']['label'] = __( 'Tagged:', 'divogue' );
			$blocks['tags']['content'] = ( ! get_the_tags() ) ? __( 'No Tags', 'divogue' ) : get_the_tag_list( '', ', ', '' );
		endif;

		if ( !empty( $display['comments'] ) && comments_open() ) :
			$blocks['comments']['label'] = __( 'With:', 'divogue' );
			ob_start();
			comments_popup_link(__( '0 Comments', 'divogue' ),
								__( '1 Comment', 'divogue' ),
								__( '% Comments', 'divogue' ), 'comments-link', '' );
			$blocks['comments']['content'] = ob_get_clean();
		endif;

		if ( $edit_link = get_edit_post_link() ) :
			$blocks['editlink']['label'] = '';
			$blocks['editlink']['content'] = '<a href="' . $edit_link . '">' . __( 'Edit This', 'divogue' ) . '</a>';
		endif;
