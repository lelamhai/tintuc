...
function hoot_appearance_subpage() {
	...
	add_theme_page(
		__( 'Divogue Premium', 'divogue' ),	// Page Title
		__( 'Premium Options', 'divogue' ),	// Menu Title
		'edit_theme_options',					// capability
		'divogue-premium',						// menu-slug
...
function hoot_admin_enqueue_upsell_styles( $hook ) {
	if ( $hook == 'appearance_page_divogue-premium' )
...
function hoot_theme_appearance_subpage() {
	...
	$hoot_cta = ( empty( $hoot_cta ) ) ? '<a class="button button-primary button-buy-premium" href="http://wphoot.com/" target="_blank">' . __( 'Click here', 'divogue' ) . '</a>' : $hoot_cta;
	...
	$hoot_intro = wp_parse_args( $hoot_intro, array(
		'name' => __('Upgrade to Premium', 'divogue'),
		'desc' => '',
		) );
	?>
