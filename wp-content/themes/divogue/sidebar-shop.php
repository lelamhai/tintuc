	...
	<aside <?php hybridextend_attr( 'sidebar', 'primary' ); ?>>
		...
		elseif ( current_user_can( 'edit_theme_options' ) ) : // If the sidebar has no widgets.
			...
			the_widget(
				'WP_Widget_Text',
				array(
					'title'  => __( 'Woocommerce Sidebar', 'divogue' ),
					/* Translators: The %s are placeholders for HTML, so the order can't be changed. */
					'text'   => sprintf( __( 'Woocommerce pages have a separate sidebar than the rest of your site. You can add custom widgets from the %swidgets screen%s in wp-admin.', 'divogue' ), '<a href="' . esc_url( admin_url( 'widgets.php' ) ) . '">', '</a>' ),
					'filter' => true,
				),
				array(
